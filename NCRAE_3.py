...
def get_model(num_users, num_items, k, layers, latent_dim, regs=[0, 0]):
    ...
    user_item_concat = Concatenate()([mul_vector, con_vector])
    ...
    prediction = prediction_layer(user_item_concat)
    # predict_vecatt = Multiply()([vec, att])

    prediction = Dense(1,
                       activation='sigmoid',
                       kernel_initializer='lecun_uniform',
                       name='prediction_classifier')(prediction)
...
if __name__ == '__main__':
    ...
    print('Init: HR@10 = %.4f, NDCG@10 = %.4f\t [%.1f s]' % (hr, ndcg, time() - t1))

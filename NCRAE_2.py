...
def parse_args():
    ...
    parser.add_argument('--lr', type=float, default=0.00001,
                        help='Learning rate.')
    parser.add_argument('--latent_layer_dim', nargs='?', default='[20, 10]',
...
def user_attention(user_latent, user_atten):
    ...
    inputs = Multiply()([user_latent, user_atten])
...
def item_attention(item_latent, item_atten):
    ...
    inputs = Multiply()([item_latent, item_atten])
    output = Dense(latent_size,
                   activation='relu',
                   kernel_initializer='glorot_normal',
                   kernel_regularizer=l2(0.001),
...
def get_model(num_users, num_items, k, layers, latent_dim, regs=[0, 0]):
    ...
    #mul_vector = Concatenate()([user_latent, item_latent])
    ...
    for idx in range(0, len(layers)):
        ...
        user_latent_atten = layer(user_latent_atten)
    ...
    for idx in range(0, len(layers)):
        ...
        item_latent_atten = layer(item_latent_atten)

    ui_mul = Multiply()([user_latent_atten, item_latent_atten])
    ui_con = Concatenate()([user_latent_atten, item_latent_atten])
    user_item_concat = Concatenate()([ui_mul, ui_con])
    #user_item_concat = Multiply()([user_latent_atten, item_latent_atten])

    Dense_layer = Dense(1,
                             activation='sigmoid',
                             kernel_initializer='lecun_normal',
                             name='prediction')
    Dense_layer = Dense_layer(user_item_concat)
    ...
    prediction = Dense(1,
                       ...
                       name='prediction_classifier')
    prediction = prediction(Dense_layer)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if epoch % verbose == 0:
            ...
            print('Iteration %d [%.1f s]: HR@10 = %.4f, NDCG@10 = %.4f, loss = %.4f [%.1f s]'

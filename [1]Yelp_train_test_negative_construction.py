...
for l_d in reviews_data:
    u_id = reviewerID2userNum.get(l_d["user_id"])
...
def split_train_test_data(train_neg_num):
    ...
    for l_train in reviews_train_data:
        user_id = reviewerID2userNum.get(l_train["user_id"])
        item_id = asin2itemNum.get(l_train["business_id"])
        label_pos = "1"
    ...
    for l_test in reviews_test_data:
        user_id = reviewerID2userNum.get(l_test["user_id"])
        item_id = asin2itemNum.get(l_test["business_id"])
        label_pos = "1"
        str_line = str(user_id) + "\t" + str(item_id) + "\t" + label_pos
...
def get_train_data():
    for d_line in reviews_data:
        user_id = reviewerID2userNum.get(d_line["user_id"])
        item_id = asin2itemNum.get(d_line["business_id"])
...
def get_test_data(num_negtives):
    test_data = {}
    t_num = 0
    for d_item in reviews_test_data:
        user_id = reviewerID2userNum.get(d_item["user_id"])
        str_pos_list = ui_dict[int(user_id)]
        ui_neg_dict[user_id] = list()
        for i in range(num_negtives):
            neg_item = np.random.randint(max_num_item)
            while str(neg_item) in str_pos_list:
                neg_item = np.random.randint(max_num_item)
...
if __name__ == '__main__':
    train_neg_num = 4
    num_negatives = 60  # 构建num_negatives的个数
